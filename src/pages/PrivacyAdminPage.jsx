...
function ageOf(iso, lang) {
  ...
  if (h < 1) return lang === "uk" ? "менш як годину тому" : "under an hour ago";
  ...
  return (
    <Modal onClose={onClose}>
      <div className="modal-h">
        <h2>{lang === "uk" ? "Схвалити запит на видалення?" : "Approve the erasure request?"}</h2>
        <p>{lang === "uk" ? "Друге підтвердження — два адміністратори" : "The second confirmation — two administrators"}</p>
      </div>
      <div className="modal-body">
        <div className="privacy-approve-copy">
          {lang === "uk"
            ? "Схвалення запускає пільговий період (типово 7 днів), після якого дані пацієнта буде безповоротно знищено, окрім того, що закон вимагає зберегти (підписані звіти, записи про згоди, конверти підписів, слід цього запиту). Протягом пільгового періоду запит ще можна скасувати."
            : "Approval starts the grace period (typically 7 days), after which the patient's data is irreversibly destroyed except what the law requires to retain (signed reports, consent records, signature envelopes, this request's paper trail). During the grace period the request can still be cancelled."}
        </div>
        {error && (
          <div className="consent-sign-error" role="alert">
            {raced
              ? (lang === "uk"
                ? "Правило двох осіб: цей запит створили ви — його має схвалити інший адміністратор."
                : "Two-person rule: you created this request — a different administrator must approve it.")
      ...
      <div className="modal-foot">
        ...
        <button className="btn accent" disabled={busy || raced} onClick={confirm}>
          {busy ? (lang === "uk" ? "Схвалення…" : "Approving…") : (lang === "uk" ? "Схвалити" : "Approve")}
        </button>
      </div>
    </Modal>
  );
    ...
    <Modal onClose={onClose}>
      <div className="modal-h">
        <h2>{cancelDuringGrace
          ? (lang === "uk" ? "Скасувати заплановане видалення?" : "Cancel the scheduled erasure?")
          : (lang === "uk" ? "Відхилити запит?" : "Reject the request?")}</h2>
        <p>{lang === "uk" ? "Причина обов'язкова — вона лишається в записі запиту" : "A reason is required — it stays on the request record"}</p>
      </div>
      <div className="modal-body">
        <textarea className="privacy-reason" rows={3} value={reason}
          onChange={(e) => setReason(e.target.value)}
          placeholder={lang === "uk" ? "Напр.: пацієнт відкликав вимогу 16.07.2026…" : "e.g. the patient withdrew the demand on 16 Jul 2026…"} />
        {error && <div className="consent-sign-error" role="alert">{error.problem?.detail || error.message}</div>}
      </div>
      <div className="modal-foot">
        <button className="btn" onClick={onClose}>{lang === "uk" ? "Назад" : "Back"}</button>
        <button className="btn" style={{ color: "var(--rec)", borderColor: "var(--rec)" }}
          ...
          {busy ? "…" : cancelDuringGrace
            ? (lang === "uk" ? "Скасувати видалення" : "Cancel the erasure")
            : (lang === "uk" ? "Відхилити запит" : "Reject the request")}
        </button>
      </div>
    </Modal>
  );
...
function DsarDetail({ req, lang, onRefresh }) {
  ...
  const download = async () => {
    ...
    try {
      ...
      setDetail(fresh);
      if (fresh.package_expired || !fresh.download) return;
      // …then stream the authenticated zip
      const blob = await downloadDsarPackage(req.id);
      ...
      if (e.status === 410) { await load(); } // package aged out mid-flow
      else setError(e);
    } finally {
  ...
  return (
    <div className="privacy-dsar-detail">
      {d.status === "executing" && (
        <span className="privacy-progress">{lang === "uk" ? "Формується пакет даних…" : "Assembling the data package…"}</span>
      )}
      ...
      {d.status === "completed" && !d.package_expired && d.download && (
        <span className="privacy-download">
          <button className="btn sm accent" disabled={downloading} onClick={download} data-testid="dsar-download">
            ...
            {downloading ? (lang === "uk" ? "Завантаження…" : "Downloading…") : (lang === "uk" ? "Завантажити пакет" : "Download package")}
          </button>
      ...
      {d.status === "completed" && d.manifest_summary && (
        <span className="privacy-manifest pmono">
          {lang === "uk" ? "обʼєктів" : "items"}: {d.manifest_summary.item_count}
          {d.manifest_summary.excluded?.length ? ` · ${lang === "uk" ? "виключено" : "excluded"}: ${d.manifest_summary.excluded.join(", ")}` : ""}
        </span>
      )}
  ...
  if (!report) {
    ...
    return (
      <div className="privacy-expired" data-testid="exec-report-unavailable">
        {lang === "uk"
          ? "Звіт про виконання сформовано на сервері, але API його ще не публікує (задача бекенду). Дані знищено згідно з правилами."
          : "The execution report exists server-side but the API does not expose it yet (named backend ask)."}
      </div>
    );
  }
...
export function PrivacyAdminPage({ lang, navigate }) {
  ...
  const erasures = requests.filter((r) => r.kind === "erasure");
  const dsars = requests.filter((r) => r.kind === "dsar");
  ...
  const patientLine = (r) => {
    ...
    return `${displayName(p, lang)}${yob ? ` · ${lang === "uk" ? "нар." : "b."} ${yob}` : ""}`;
  };
  ...
  const renderRow = (r) => {
    ...
    return (
      <div key={r.id} className={`privacy-row status-${r.status}`} data-own={own || undefined}>
        <div className="privacy-row-main" onClick={() => setExpanded((m) => ({ ...m, [r.id]: !open }))}>
          <div className="privacy-row-what">
            <span className={`privacy-kind ${r.kind}`}>
              {r.kind === "erasure" ? (lang === "uk" ? "Видалення" : "Erasure") : "DSAR"}
            </span>
            <strong>{patientLine(r)}</strong>
            ...
            {own && ["requested", "review"].includes(r.status) && r.kind === "erasure" && (
              <span className="privacy-own-badge" data-testid="own-request-badge">
                {lang === "uk" ? "ваш запит — очікує на розгляд іншим адміністратором" : "your request — awaiting review by another administrator"}
              </span>
            )}
          </div>
          <div className="privacy-row-meta">
            <span>{lang === "uk" ? "створено" : "created"} {ageOf(r.requested_at, lang)}</span>
            {r.reason && <span className="privacy-reason-preview">«{r.reason.slice(0, 80)}{r.reason.length > 80 ? "…" : ""}»</span>}
          </div>
        </div>

        <div className="privacy-row-side">
          ...
          {actionable && !own && (
            <>
              <button className="btn sm accent" data-testid="approve-btn" onClick={() => setApproveTarget(r)}>
                {lang === "uk" ? "Схвалити" : "Approve"}
              </button>
              <button className="btn sm" data-testid="reject-btn" onClick={() => setRejectTarget({ req: r, grace: false })}>
                {lang === "uk" ? "Відхилити" : "Reject"}
              </button>
            </>
          )}
          {r.status === "rejected" && r.rejection_reason && (
            <span className="privacy-rejected-reason" title={r.rejection_reason}>
              {lang === "uk" ? "причина: " : "reason: "}«{r.rejection_reason.slice(0, 60)}»
            </span>
          )}
        </div>
  ...
  return (
    <div className="page privacy-queue">
      <div className="page-h">
        <div style={{ flex: 1 }}>
          <h1>{lang === "uk" ? "Приватність" : "Privacy"}</h1>
          <p className="sub">
            {lang === "uk"
              ? "Запити DSAR та запити на видалення даних. Видалення схвалюється за правилом двох осіб."
              : "DSAR and erasure requests. Erasure follows the two-person rule."}
          </p>
        </div>
        <button className="btn" onClick={listReq.reload}>
          <Icon name="refresh" size={13} /> {lang === "uk" ? "Оновити" : "Refresh"}
        </button>
      </div>

      {listReq.loading && <Loading lang={lang} />}
      {listReq.error && <ApiErrorView error={listReq.error} lang={lang} />}
      {!listReq.loading && !listReq.error && requests.length === 0 && (
        <Empty icon="shield" title={lang === "uk" ? "Запитів немає" : "No privacy requests"} />
      )}

      {erasures.length > 0 && (
        <section className="privacy-section">
          <h3>{lang === "uk" ? "Запити на видалення" : "Erasure requests"}</h3>
          {erasures.map(renderRow)}
        </section>
      )}
      {dsars.length > 0 && (
        <section className="privacy-section">
          <h3>{lang === "uk" ? "Запити DSAR" : "DSAR requests"}</h3>
          {dsars.map(renderRow)}
        </section>
      )}
