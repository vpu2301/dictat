...
// and issues an envelope with signature_level='dev' (NOT qualified). It
// is gated on Vite's DEV flag so it is tree-shaken out of production
// builds entirely — matching the backend's SIGNING_DEV_PASSWORD_ENABLED
// guard. Remove both before production.
...
// ── Password flow (DEV / PILOT scaffold) ──────────────────────────────────

function DevPasswordFlow({ reportId, onSuccess, onCancel }) {
  const { lang } = useI18n()
  const uk = lang === 'uk'
...
export function SigningFlow({ reportId, report, onClose, onSigned, lang }) {
  ...
  if (phase === 'password') {
    return <DevPasswordFlow reportId={reportId} onSuccess={handleSuccess} onCancel={onClose} />
  }
  if (phase === 'success') {
